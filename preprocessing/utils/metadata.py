...
def prepare_metadata(metadata_file: str) -> "pd.DataFrame":
    ...
    galvanized = ["verzinkt"]
    brushed = ["blank", "schwarz gebürstet"]
    burnished = ["brüniert"]
    # COLOR
    black = ["schwarz", "schwarz eingefärbt", "schwarz eloxiert", "schwarz gebürstet"]
    grey = ["grau"]
    ...
    for material, surface in DEFAULT_SURFACE.items():
        df.loc[df.part_material == material, "part_surface"] = df.loc[
            df.part_material == material, "part_surface"
        ].replace("-", surface, inplace=False)
